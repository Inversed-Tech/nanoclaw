...
4. Under **OAuth & Permissions**, add Bot Token Scopes:
   - `chat:write`, `channels:history`, `groups:history`, `im:history`, `channels:read`, `groups:read`, `users:read`, `files:read`
...
- **Image support requires `files:read` scope** — The bot downloads image attachments (PNG, JPEG, GIF, WebP up to 10 MB). If missing, images are silently skipped but the text message is still delivered.
